...
(function () {
  ...
  function initMatchRush() {
    ...
    function endGame(isVictory = false) {
      ...
      const highScores = JSON.parse(localStorage.getItem("grammar_phrasal_high_scores") || "{}");
      const currentBest = highScores[gameMode] || 0;
      let isNewHigh = false;

      if (score > currentBest) {
        highScores[gameMode] = score;
        localStorage.setItem("grammar_phrasal_high_scores", JSON.stringify(highScores));
        isNewHigh = true;
      }
  ...
  try { if(window.LAStars){LAStars.recordPlay("grammar-phrasal-verbs");var acc=typeof accuracy!=="undefined"?accuracy:0;LAStars.saveFromAccuracy("grammar-phrasal-verbs",acc);} } catch (e) {}
}

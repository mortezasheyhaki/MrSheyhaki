...
var GAME_ID = "starter-9b-clothes-sentence-challenge";
  ...
  function placeWord(slot, word) {
    ...
    if (currentSlots.every(Boolean) && currentSlots.every(function(v){ return String(v).split(/\s+/).some(isClothing); })) {
      ...
      if (correctSentence) {
        part1Correct++;
        if (feedback) { feedback.textContent = "✓ Excellent! The sentence is correct."; try{sfxCorrect();}catch(e){} feedback.className = "csc-feedback good"; }
        sfx("good");
        setTimeout(function () {
  ...
  function checkTyped() {
    ...
    if (ok) { try{sfxCorrect();}catch(e){}
      part2Correct++;
      sfx("good");
      feedback.textContent = "✓ Perfect sentence!"; try{sfxCorrect();}catch(e){}
      ...
      sfx("bad");
      feedback.textContent = "Try again. Look carefully at the picture and the question."; try{sfxWrong();}catch(e){}

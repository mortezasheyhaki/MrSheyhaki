/**
 * Sentence Builder · be — Starter Unit 1A
 * Modes: Positive → Negative → Questions
 * Full form then short form (positive/negative); questions + short answers.
 */
(function () {
  "use strict";

  const GAME_ID = "starter-1a-sentence-builder-be";
  const AUDIO = "audio/";
  ...
  const DATA = {
    positive: {
      label: "Positive",
      icon: "➕",
      desc: "Full form → short form (contractions)",
      phases: [
        {
          name: "Full form",
          items: [
            {
              chips: ["I", "am", "Helen."],
              answer: ["I", "am", "Helen."],
              audio: "i-am-helen.mp3",
            },
            {
              chips: ["You", "are", "Tom."],
              answer: ["You", "are", "Tom."],
              audio: "you-are-tom.mp3",
            },
          ],
        },
        {
      ...
      phases: [
        {
          ...
          items: [
            {
              chips: ["I", "am", "not", "Ellen."],
              answer: ["I", "am", "not", "Ellen."],
              audio: "i-am-not-ellen.mp3",
            },
            {
              chips: ["You", "are", "not", "Dom."],
              answer: ["You", "are", "not", "Dom."],
              audio: "you-are-not-dom.mp3",
            },
          ],
        },
        {
          ...
          items: [
            {
              chips: ["I", "'m", "not", "Ellen."],
              answer: ["I", "'m", "not", "Ellen."],
              audio: "im-not-ellen.mp3",
            },
            {
              chips: ["You", "aren't", "Dom."],
              answer: ["You", "aren't", "Dom."],
              audio: "you-arent-dom.mp3",
            },
          ],
        },
      ],
    ...
    questions: {
      label: "Questions",
      icon: "❓",
      desc: "Make questions and short answers",
      phases: [
        {
          name: "Questions",
          items: [
            {
              chips: ["Am", "I", "in", "room", "2?"],
              answer: ["Am", "I", "in", "room", "2?"],
              audio: "am-i-in-room-2.mp3",
            },
            {
              chips: ["Are", "you", "Mike?"],
              answer: ["Are", "you", "Mike?"],
              audio: "are-you-mike.mp3",
            },
          ],
        },
        {
  ...
  function showStart() {
    ...
    app.innerHTML = `
      ...
      <div class="sb-start">
        ...
        <p>Build sentences with <strong>be</strong> — positive, negative, and questions.</p>
        <div class="sb-modes">
  ...
  function renderItem() {
    ...
    const pool = shuffle(item.chips.slice());
    selected = [];
    locked = false;

    let promptExtra;
    if (item.prompt) {
      // e.g. "Am I in room 2? → Yes" → show question clearly + answer hint
      ...
      promptExtra = `
        ...
        <div class="sb-prompt-label">Build the short answer</div>`;
    } else {
      promptExtra = `<div class="sb-prompt-label">Tap the chips in order</div>`;
    }
      ...
      <div class="sb-play">
        ...
        <div class="sb-prompt">
          ...
          <div class="sb-slots" id="sb-slots">
            ${item.answer.map(() => `<span class="sb-slot"></span>`).join("")}
          </div>
        </div>
        <div class="sb-chips" id="sb-chips">
          ${pool
            .map(
              (c, i) =>
                `<button type="button" class="sb-chip" data-i="${i}" data-text="${escapeAttr(c)}">${escapeHtml(c)}</button>`
            )
            .join("")}
        </div>
        <div class="sb-actions">
          <button type="button" class="sb-btn sb-btn-ghost" id="sb-clear">Clear</button>
          <button type="button" class="sb-btn sb-btn-primary" id="sb-check" disabled>Check ✓</button>
        </div>
      </div>`;
    ...
    document.getElementById("sb-clear").addEventListener("click", () => {
      ...
      selected = [];
      updateSlots();
      app.querySelectorAll(".sb-chip").forEach((b) => {
        b.disabled = false;
    ...
    app.querySelectorAll(".sb-chip").forEach((btn) => {
      btn.addEventListener("click", () => {
        if (locked || btn.disabled) return;
        if (selected.length >= item.answer.length) return;
        sfxTap();
        selected.push(btn.dataset.text);
        btn.disabled = true;
        btn.classList.add("used");
        updateSlots();
        document.getElementById("sb-check").disabled = selected.length !== item.answer.length;
      });
    });

    // Tap filled slot to undo last matching chip
    document.getElementById("sb-slots").addEventListener("click", (e) => {
      if (locked) return;
      const slot = e.target.closest(".sb-slot.filled");
      if (!slot) return;
      const idx = [...slot.parentNode.children].indexOf(slot);
      if (idx !== selected.length - 1) return; // only undo last
      const text = selected.pop();
      updateSlots();
      const chip = [...app.querySelectorAll(".sb-chip.used")].reverse().find((c) => c.dataset.text === text);
      if (chip) {
        chip.disabled = false;
  ...
  function updateSlots() {
    const slots = app.querySelectorAll(".sb-slot");
    slots.forEach((slot, i) => {
      slot.classList.remove("filled", "correct", "wrong");
      slot.textContent = selected[i] || "";
      if (selected[i]) slot.classList.add("filled");
    });
  }
  ...
  function checkAnswer() {
    ...
    const ok = selected.every((t, i) => t === item.answer[i]);
    const slots = app.querySelectorAll(".sb-slot");
    slots.forEach((slot) => {
  ...
  function advance() {
    ...
    showCelebrate(
      praise(),
      `${DATA[mode].label} complete! Try another mode.`,
      () => showStart(),
      remaining[0]
    );
  }
